      ...
      <!-- Main Content-->
        <div class="main-content pt-0">
          <div class="container">
                      ...
                      <div class="card custom-card">
                          <div class="card-body h-100">
                              <div class="text-center">
                                @if(auth()->user()->isPaid())
                                  <button data-toggle="modal" class="btn ripple btn-primary" href="#create_group_model" style="margin-top: 5px; margin-bottom: 5px; font-weight: bold;text-transform: uppercase;">Opret GRUPPE</button>
                                @else
                                  <button data-toggle="modal" class="btn ripple btn-primary" href="#create_group_model" style="margin-top: 5px; margin-bottom: 5px; font-weight: bold;text-transform: uppercase;" disabled>Opret GRUPPE</button>
                                @endif
                              </div>
                          </div>
                      </div>
                          ...
                          <div class="card-body h-100">
                            <div class="row">
                              ...
                              <div class="col-sm-9 col-lg-9">
                                  <form method="POST" action="{{route('groupsearch')}}">
                                      @csrf
                                    ...
                                    <div class="product h-100">
                                      <!-- Item Body -->
                                        <div class="text-center product-img mb-0">
                                          <a href="{{route('groupDetails',$d->id)}}" >
                                      ...
                                      <!-- Action Link -->
                                        <div class="product-info">
                                          <a href="{{route('groupDetails',$d->id)}}" class="btn ripple  btn-primary btn-sm mt-1 mb-1 text-sm text-white"  data-toggle="tooltip" data-placement="bottom" title="Udsigt">
                                            <i class="fe fe-eye"></i>
                                          </a>

                                          @if(Auth::user()->id != $d->user_id) 
                                            @if(App\Models\Groups::checkThisMemberAreInThisGroup(Auth::user()->id,$d->id) || App\Models\Groups::checkThisMemberAreInThisGroupPending(Auth::user()->id,$d->id))
                                            
                                                @if(App\Models\Groups::checkThisMemberAreInThisGroupPending(Auth::user()->id,$d->id))
                                                <a href="{{route('joinGroup',$d->id)}}" class="btn ripple  btn-sm btn-secondary mt-1 text-sm  mb-1 text-white" data-toggle="tooltip" data-placement="bottom" title="Afventer">
                                                  <i class="fe fe-plus-square"></i>
                                                </a>
                                                @else
                                                  <a href="{{route('joinGroup',$d->id)}}" class="btn ripple  btn-sm btn-secondary mt-1 text-sm  mb-1 text-white" data-toggle="tooltip" data-placement="bottom" title="Forlad">
                                                  <i class="fe fe-plus-square"></i>
                                                </a>
                                                @endif
                                            @else
                                              <a @if(auth()->user()->isPaid()) href="{{route('joinGroup',$d->id)}}"@endif @if(!auth()->user()->isPaid()) style="cursor: not-allowed;"@endif class="btn ripple  btn-sm btn-secondary mt-1 text-sm  mb-1 text-white" data-toggle="tooltip" data-placement="bottom" title="Tilmeld">
                                                <i class="fe fe-plus-square"></i>
                                              </a>
                                            @endif
                                          @endif
                                          ...
                                          <a href="{{route('group.groupedit',$d->id)}}" class="btn ripple  btn-info btn-sm mt-1 mb-1 text-sm  text-white"  data-toggle="tooltip" data-placement="bottom" title="Redigere">
                                            <i class="fe fe-edit"></i>
                                          </a>
                                          @endif
                                          @if( (auth()->user()->isPaid()) && (Auth::user()->id == $d->user_id) )
                                                <button class="btn ripple  btn-sm btn-danger mt-1 text-sm  mb-1 text-white" type="button" onclick="deactivegroup({{ $d->id }})">
                                                    <i class="fe fe-trash"></i>
                                                </button>
                                                <form id="delete-form-{{$d->id}}" action="{{ route('group.groupdeactive',$d->id) }}" method="POST" style="display: none;">
                                                    @csrf
                                                    <!-- @method('DELETE') -->
                                                </form>
            ...
            <!-- Create Group Modal -->
                      <div class="modal" id="create_group_model">
                          <div class="modal-dialog modal-lg" role="document">
                              <div class="modal-content modal-content-demo">
                                  <div class="modal-header">
                                      <h6 class="modal-title" style="font-weight: bold;text-transform: uppercase;">Opret GRUPPE</h6><button aria-label="Close" class="close" data-dismiss="modal" type="button"><span aria-hidden="true">&times;</span></button>
                                  </div>
                                  ...
                                  <!-- Event Data -->  
                                    <form id="form_event_create" method="POST" action="{{ route('group.groupstore') }}">
                                        @csrf
                                        <div class="modal-body">
                                            <div class="row">
                                                <div class="col-sm-12 col-md-6">
                                                ...
                                                <div class="col-sm-12 col-md-6">
                                                  <div class="form-group">
                                                      ...
                                                      <select style="width: 100%" name="group_type" class="form-control select2" required>
                                        ...
                                        <div class="modal-footer">
                                            <button class="btn ripple btn-success" type="submit" id="form_submit" style="font-weight: bold;text-transform: uppercase;">Opret @if(Auth::user()->getportal(Auth::user()->portalJoinUser_id) == 3 || Auth::user()->getportal(Auth::user()->portalJoinUser_id) == 4 || Auth::user()->getportal(Auth::user()->portalJoinUser_id) == 6) Loger @else gruppe @endif</button>
